...
function renderDetails(shipment) {
  ...
  const rows = [
    ...
    ['Pickup date', shipment.pickupDate], ['Delivery', [shipment.deliveryDate, shipment.deliveryTime, shipment.deliveryLocation].filter(Boolean).join(' ')]
  ].filter(([, value]) => value);
  const history = events.length ? events.map((event, index) => {
    const eventStatus = statusMeta(event.status || shipment.status);
    return `<li class="details-timeline-item ${index === 0 ? 'is-current' : ''}"><span class="details-timeline-icon ${eventStatus[1]}">${eventStatus[2]}</span><div><div class="details-timeline-top"><strong>${escapeDetailsHtml(event.status || shipment.status || 'Update')}</strong><time>${escapeDetailsHtml(formatDate(event))}${formatTime(event) ? ` · ${escapeDetailsHtml(formatTime(event))}` : ''}</time></div><p>${escapeDetailsHtml(event.description || event.remarks || 'Shipment update')}</p>${event.location ? `<span class="details-timeline-location">${escapeDetailsHtml(event.location)}</span>` : ''}</div></li>`;
